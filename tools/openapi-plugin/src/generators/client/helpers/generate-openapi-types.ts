...
/**
 * Called when this file is called as a command from the nx-executor
 * @param args
 */
function regenerateClient(...args: string[]) {
  const tree: Tree = JSON.parse(args[1] as string); // TODO: How to get around this
  // TODO: (MI-94) Pass the schemaPath and remote options seperately from the generator. That way you can generate from a new source
  console.log('Regenerating from pre-existing schema...');
  await generateOpenApiTypes(tree, options);
}

regenerateClient(...process.argv);
